...
BEGIN_EVENT_TABLE(CommitDlg, wxDialog)
	...
	EVT_BUTTON(ID_BUTTON_CANCEL, CommitDlg::OnCancel)
END_EVENT_TABLE()

void CommitDlg::OnOk( wxCommandEvent& WXUNUSED(event) )
{
	EndModal( ID_BUTTON_OK );
}

void CommitDlg::OnCancel( wxCommandEvent& WXUNUSED(event) )
{
	EndModal( ID_BUTTON_CANCEL );
}

void CommitDlg::InitializeData()
{
