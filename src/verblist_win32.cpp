...
void VerbList::InitFromDocument (const wxString &document_path)
{
  ...
  if (wxDirExists (m_document_path.GetFullPath ()))
  {
  ...
  if (progid_key.Open (HKEY_CLASSES_ROOT, progid_key_name.c_str ()))
  {
    ...
    progid_key.GetStringValue (NULL, m_document_type_name);
      ...
      while (verb_base_key.GetNextSubKeyName (verb_key_name))
      {
        try
        {
          ...
          if (verb_key_name == default_verb_name)
            m_verb_list.insert (m_verb_list.begin (), verb);
          else
            m_verb_list.push_back (verb);
        }
...
{
  return m_verb_list.size ();
}

const char *VerbList::GetName (size_t index) const
{
  return m_verb_list[index].m_name.c_str ();
}

void VerbList::Launch (size_t index) const
{
  m_verb_list[index].Launch (m_document_path.GetFullPath ().c_str ());
}

/****************************************************************************/

VerbList::Verb::Verb () : m_uses_dde (false)
{
}

void VerbList::Verb::ReadFromRegistry (const RegKey &base, LPCSTR verb_name)
{
...
void VerbList::Verb::Launch (const char *document_path) const
{
