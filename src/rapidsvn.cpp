...
VSvnFrame::~VSvnFrame()
{
	...
	pConfig->Write( "/MainFrame/vertsplitsashpos", (long)m_vert_splitter->GetSashPosition() );
	pConfig->Write( "/MainFrame/horizsplitsashpos", (long)m_horiz_splitter->GetSashPosition() );
...
void VSvnFrame::OnAbout(wxCommandEvent& WXUNUSED(event))
{
	wxMessageBox( "RapidSVN\nBuilt with Subversion Alpha\nand wxWindows 2.3.2",
				"About VSvn", wxOK | wxICON_INFORMATION );
}

svn_client_auth_baton_t * svn_cl__make_auth_baton ( apr_pool_t *pool)
{
	svn_client_auth_baton_t *auth_obj;
	auth_obj = (svn_client_auth_baton_t*)apr_pcalloc (pool, sizeof(*auth_obj));
...
void VSvnFrame::OnStatus(wxCommandEvent& WXUNUSED(event))
{
	...
	for ( ;; )
	{
		...
		if ( item == -1 )
			break;
#ifdef WIN32
		fullPath = m_folder_browser->GetPath() + "\\" + m_listCtrl->GetItemText( item );
		UnixPath( fullPath );
#else
		fullPath = m_folder_browser->GetPath() + "/" + m_listCtrl->GetItemText( item );
#endif
...
void VSvnFrame::OnInfo(wxCommandEvent& WXUNUSED(event))
{
	ShowInfo();
}

void VSvnFrame::OnLog(wxCommandEvent& WXUNUSED(event))
{
	ShowLog();
}

void VSvnFrame::OnBrowse(wxCommandEvent& WXUNUSED(event))
{
	BrowseDir();
}

void VSvnFrame::OnCombo(wxCommandEvent& event)
{
	wxFileName		fullpath( event.GetString() );
	wxFileName		treepath( wxGetApp().GetAppFrame()->GetFolderBrowser()->GetPath() );
	...
	{	
		...
		wxLogStatus(_T("Working copy '%s' selected"), event.GetString().c_str());
	}	
}

void VSvnFrame::OnImport( wxCommandEvent& WXUNUSED(event) )
{
...
void VSvnFrame::OnCheckout( wxCommandEvent& WXUNUSED(event) )
{
...
void VSvnFrame::OnAdd( wxCommandEvent& WXUNUSED(event) )
{
	AddEntries();
}

void VSvnFrame::OnDelete( wxCommandEvent& WXUNUSED(event) )
{
	DelEntries();
}

void VSvnFrame::OnCommit( wxCommandEvent& WXUNUSED(event) )
{
	MakeCommit();
}

void VSvnFrame::OnRevert( wxCommandEvent& WXUNUSED(event) )
{
	MakeRevert();
}

void VSvnFrame::OnResolve( wxCommandEvent& WXUNUSED(event) )
{
	MakeResolve();
}

void VSvnFrame::OnCopy( wxCommandEvent& WXUNUSED(event) )
{
	MakeCopy();
}

void VSvnFrame::OnMkdir( wxCommandEvent& WXUNUSED(event) )
{
	Mkdir();
}

void VSvnFrame::OnMerge( wxCommandEvent& WXUNUSED(event) )
{
	Merge();
}

void VSvnFrame::OnContents( wxCommandEvent& WXUNUSED(event) )
{
	Contents();
}


void VSvnFrame::LayoutChildren()
{
...
void VSvnFrame::OnSize(wxSizeEvent& event)
{
	if ( m_tbar )
	{
		LayoutChildren();
...
void VSvnFrame::RecreateToolbar()
{
...
void VSvnFrame::AddBrowseTools()
{
	...
	m_comboBrowse = new wxComboBox( toolBar, 
									ID_Combo,
									_T( "" ),
									wxDefaultPosition,
									wxSize(180, -1),
									0,				// number of strings to initialise the ctrl
...
void VSvnFrame::AddActionTools()
{
	...
	toolBar->AddTool( TOOLBAR_UPDATE, 
						...
						(wxObject *)NULL,
						_T("Update selected"),
						_T("Bring changes from the repository into the working copy") );
	...
	toolBar->AddTool( TOOLBAR_COMMIT, 
						...
						(wxObject *)NULL,
						_T("Commit selected"),
						_T("Send changes from your working copy to the repository") );
	...
	toolBar->AddTool( TOOLBAR_REVERT, 
						...
						(wxObject *)NULL,
						_T("Revert selected"),
						_T("Restore pristine working copy file (undo all local edits)") );
	...
	toolBar->AddTool( TOOLBAR_RESOLVE, 
						...
						(wxObject *)NULL,
						_T("Resolve selected"),
						_T(" Remove 'conflicted' state on working copy files or directories)") );

	toolBar->AddSeparator();
}

void VSvnFrame::AddInfoTools()
{
	wxToolBarBase *toolBar = GetToolBar();
	...
	toolBar->AddTool( TOOLBAR_STATUS, 
						...
						(wxObject *)NULL,
						_T("Status selected"),
						_T("Print the status of working copy files and directories") );
...
void VSvnFrame::OnToolEnter( wxCommandEvent& event )
{
}

void VSvnFrame::OnToolLeftClick( wxCommandEvent& event )
{
	switch( event.GetId() )
	{
...
void VSvnFrame::BrowseDir()
{
	...
	if( dialog.ShowModal() == wxID_OK )
	{
		...
		wxLogStatus( _T("Added working directory '%s'"), dialog.GetPath().c_str() );
	}
}

void VSvnFrame::InitComboBrowser()
{
	wxConfigBase *pConfig = wxConfigBase::Get();
...
void VSvnFrame::OnUpdate( wxCommandEvent& event )
{
	MakeUpdate();
}

void VSvnFrame::OnActionEvent( wxCommandEvent& event )
{
	switch( event.GetInt() )
	{
		case TOKEN_INFO:
		...
		case TOKEN_SVN_INTERNAL_ERROR:
			{
				if( event.GetClientData() )
				{
					...
					handle_svn_error( (svn_error_t*)event.GetClientData(),
										&err_tr );
					err_tr.ShowErrors();
				}
			}
...
void VSvnFrame::MakeRevert()
{
	wxMessageDialog		sure_dlg( this, 
					_T("Are you sure you want to revert local changes to pristine files?"),
					_T("Revert"),
...
void VSvnFrame::MakeResolve()
{
	...
	ResolveAction	*add_act = new ResolveAction( this, pool, m_logTracer, targets );
	add_act->Perform();
}

void VSvnFrame::DelEntries()
{
	if( m_listCtrl->GetSelectedItemCount() == 0 )
		return;
...
void LogTracer::Trace( const wxString& str )
{
	AppendText( str + _T("\n") );
}
