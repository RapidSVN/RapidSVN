...
static void GetRevFromString( svn_client_revision_t *rev, wxString str )
{
	unsigned long	nr;
	TrimString( str );
...
void* MergeAction::Entry()
{
	...
	wxString		targetPath = wxGetApp().GetAppFrame()->GetFolderBrowser()->GetPath();
	...
	err = svn_wc_get_actual_target( targetPath.c_str(), &parent_dir, &entry, pool );
	if( err )
	{
...
void MergeAction::GetNotifier(svn_wc_notify_func_t *notify_func_p, void **notify_baton_p, svn_boolean_t is_checkout, svn_boolean_t suppress_final_line, apr_pool_t *pool)
{
