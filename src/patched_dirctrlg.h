...
{
public:
    PatchedDirItemData(const wxString& path, const wxString& name, bool isDir);
    ~PatchedDirItemData();
...
public:
    ...
    virtual PatchedDirFilterListCtrl* GetFilterListCtrl() const { return m_filterListCtrl; }
...
private:
    DECLARE_EVENT_TABLE()
    DECLARE_DYNAMIC_CLASS(PatchedGenericDirCtrl)
};
...
public:
    ...
    PatchedDirFilterListCtrl(PatchedGenericDirCtrl* parent, const wxWindowID id = -1,
              const wxPoint& pos = wxDefaultPosition,
              const wxSize& size = wxDefaultSize,
              long style = 0)
    {
...
protected:
    ...
    DECLARE_EVENT_TABLE()
    DECLARE_CLASS(PatchedDirFilterListCtrl)
};
