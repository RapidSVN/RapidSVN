...
static bool isDriveAvailable(const wxString& dirName)
{
...
static int dirCtrlStringCompareFunction(const void *first, const void *second)
{
...
PatchedDirItemData::PatchedDirItemData(const wxString& path, const wxString& name,
				       bool isDir)
{
    m_path = path;
    m_name = name;
...
void PatchedDirItemData::SetNewDirName(const wxString& path)
{
    m_path = path;
    m_name = wxFileNameFromPath(path);
}

bool PatchedDirItemData::HasSubDirs() const
{
    if (m_path.IsEmpty())
        return FALSE;
...
bool PatchedDirItemData::HasFiles(const wxString& WXUNUSED(spec)) const
{
    if (m_path.IsEmpty())
        return FALSE;
...
BEGIN_EVENT_TABLE(PatchedGenericDirCtrl, wxControl)
  ...
  EVT_SIZE                    (PatchedGenericDirCtrl::OnSize)
END_EVENT_TABLE()
...
bool PatchedGenericDirCtrl::Create(wxWindow *parent,
                              const wxWindowID id,
                              const wxString& dir,
                              const wxPoint& pos,
                              const wxSize& size,
                              ...
                              const wxString& name)
{
    ...
    if (!filter.IsEmpty() && (style & PATCHED_DIRCTRL_SHOW_FILTERS))
        m_filterListCtrl = new PatchedDirFilterListCtrl(this, wxID_FILTERLISTCTRL, wxDefaultPosition, wxDefaultSize, filterStyle);
    ...
    m_showHidden = FALSE;
    PatchedDirItemData* rootData = new PatchedDirItemData(wxT(""), wxT(""), TRUE);
...
void PatchedGenericDirCtrl::Init()
{
    m_showHidden = FALSE;
    m_imageList = NULL;
    m_currentFilter = 0;
...
void PatchedGenericDirCtrl::ShowHidden( bool show )
{
    m_showHidden = show;
...
const wxTreeItemId PatchedGenericDirCtrl::AddSection(const wxString& path, const wxString& name, int imageId)
{
    PatchedDirItemData *dir_item = new PatchedDirItemData(path,name,TRUE);

    const wxTreeItemId id = m_treeCtrl->AppendItem( m_rootId, name, imageId, -1, dir_item);
...
void PatchedGenericDirCtrl::SetupSections()
{
...
void PatchedGenericDirCtrl::OnBeginEditItem(wxTreeEvent &event)
{
...
void PatchedGenericDirCtrl::OnEndEditItem(wxTreeEvent &event)
{
    ...
    PatchedDirItemData *data = (PatchedDirItemData*)m_treeCtrl->GetItemData( id );
    wxASSERT( data );
...
void PatchedGenericDirCtrl::OnExpandItem(wxTreeEvent &event)
{
...
void PatchedGenericDirCtrl::OnCollapseItem(wxTreeEvent &event )
{
    CollapseDir(event.GetItem());
}

void PatchedGenericDirCtrl::CollapseDir(const wxTreeItemId& parentId)
{
    wxTreeItemId child;

    PatchedDirItemData *data = (PatchedDirItemData *) m_treeCtrl->GetItemData(parentId);
    if (!data)
	return;
...
void PatchedGenericDirCtrl::ExpandDir(const wxTreeItemId& parentId)
{
    PatchedDirItemData *data = (PatchedDirItemData *) m_treeCtrl->GetItemData(parentId);
    ...
    if ((GetWindowStyle() & PATCHED_DIRCTRL_DIR_ONLY) == 0)
    {
        ...
        filenames.Sort((wxArrayString::CompareFunction) dirCtrlStringCompareFunction);
    }
    ...
    for (i = 0; i < dirs.Count(); i++)
    {
        ...
        PatchedDirItemData *dir_item = new PatchedDirItemData(path,eachFilename,TRUE);
    ...
    if ((GetWindowStyle() & PATCHED_DIRCTRL_DIR_ONLY) == 0)
    {
        for (i = 0; i < filenames.Count(); i++)
        {
            ...
            PatchedDirItemData *dir_item = new PatchedDirItemData(path,eachFilename,FALSE);
            (void)m_treeCtrl->AppendItem( parentId, eachFilename, 2, -1, dir_item);
        }
    }
}

void PatchedGenericDirCtrl::ReCreateTree()
{
...
wxTreeItemId PatchedGenericDirCtrl::FindChild(wxTreeItemId parentId, const wxString& path, bool& done)
{
    wxString path2(path);
    ...
    while (childId.IsOk())
    {
        PatchedDirItemData* data = (PatchedDirItemData*) m_treeCtrl->GetItemData(childId);
...
bool PatchedGenericDirCtrl::ExpandPath(const wxString& path)
{
    bool done = FALSE;
    ...
    if (lastId.IsOk())
    {
        PatchedDirItemData *data = (PatchedDirItemData *) m_treeCtrl->GetItemData(lastId);
        if (data->m_isDir)
        {
...
wxString PatchedGenericDirCtrl::GetPath() const
{
...
wxString PatchedGenericDirCtrl::GetFilePath() const
{
    wxTreeItemId id = m_treeCtrl->GetSelection();
    if (id)
    {
...
void PatchedGenericDirCtrl::SetPath(const wxString& path)
{
    m_defaultPath = path;
    if (m_rootId)
        ExpandPath(path);
...
// Not used
#if 0
void PatchedGenericDirCtrl::FindChildFiles(wxTreeItemId id, int dirFlags, wxArrayString& filenames)
{
    PatchedDirItemData *data = (PatchedDirItemData *) m_treeCtrl->GetItemData(id);
...
void PatchedGenericDirCtrl::SetFilterIndex(int n)
{
    m_currentFilter = n;
...
void PatchedGenericDirCtrl::SetFilter(const wxString& filter)
{
    m_filter = filter;
...
bool PatchedGenericDirCtrl::ExtractWildcard(const wxString& filterStr, int n, wxString& filter, wxString& description)
{
...
int PatchedGenericDirCtrl::ParseFilter(const wxString& filterStr, wxArrayString& filters, wxArrayString& descriptions)
{
...
void PatchedGenericDirCtrl::DoResize()
{
...
void PatchedGenericDirCtrl::OnSize(wxSizeEvent& WXUNUSED(event))
{
    DoResize();
}
...
BEGIN_EVENT_TABLE(PatchedDirFilterListCtrl, wxChoice)
    EVT_CHOICE(-1, PatchedDirFilterListCtrl::OnSelFilter)
END_EVENT_TABLE()

bool PatchedDirFilterListCtrl::Create(PatchedGenericDirCtrl* parent, const wxWindowID id,
              const wxPoint& pos,
              const wxSize& size,
              long style)
{
...
void PatchedDirFilterListCtrl::Init()
{
    m_dirCtrl = NULL;
}

void PatchedDirFilterListCtrl::OnSelFilter(wxCommandEvent& WXUNUSED(event))
{
...
void PatchedDirFilterListCtrl::FillFilterList(const wxString& filter, int defaultFilter)
{
    Clear();
