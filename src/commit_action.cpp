...
void* CommitAction::Entry()
{
	...
	if( (!targets) || (!targets->nelts) )
	{
		...
		err = svn_wc_get_actual_target( base_dir, &parent_dir, 
										&base_name, pool);
		if (base_name)
	...
	err = svn_client_commit( &commit_info,
							...
							svn_cl__make_log_msg_baton( logMsg.c_str(), base_dir, pool ),
							NULL,
							revnum,
							!recursive,		// non-recursive status
							pool );
	...
	{
		wxString	str = wxString::Format( "Committed revision %" SVN_REVNUM_T_FMT ".",
