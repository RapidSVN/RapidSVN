...
EVT_BUTTON (ID_BUTTON_BROWSE_SRC, CopyDlg::OnBrowseSrc) END_EVENT_TABLE ()
     void
     CopyDlg::OnOk (wxCommandEvent & WXUNUSED (event))
{
  wxString val;
  unsigned long rev = 0;
...
void
CopyDlg::OnCancel (wxCommandEvent & WXUNUSED (event))
{
  EndModal (ID_BUTTON_CANCEL);
}

void
CopyDlg::OnBrowseSrc (wxCommandEvent & event)
{
...
void
CopyDlg::OnBrowseDest (wxCommandEvent & event)
{
  wxDirDialog dialog (this,
