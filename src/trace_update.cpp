...
static DirBaton* make_dir_baton( const char *path,
								void *edit_baton,
...
static FileBaton* make_file_baton( const char *path,
											void *parent_dir_baton,
											svn_boolean_t added,
											apr_pool_t *pool )
{
									...
									apr_pool_t *pool )
{
	...
	
	{
		...
		wxString str = wxString::Format( "D  %s", svn_path_join( eb->path, path, pool ) );
		eb->tracer->Trace( str );
	}
	return SVN_NO_ERROR;
}
