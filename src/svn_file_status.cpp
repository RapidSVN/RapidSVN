...
void SvnFileStatus::retrieveStatus( const wxString &path, AuthBaton& auth_baton )
{
...
svn_wc_status_kind SvnFileStatus::getFileStatus() const
{
	if( !status )
...
svn_wc_status_kind SvnFileStatus::getFilePropStatus() const
{
	if( !status )
		return svn_wc_status_none;
...
svn_revnum_t SvnFileStatus::getRevision() const
{
...
svn_revnum_t SvnFileStatus::getLastChange() const
{
	if( !status || !status->entry )
		return SVN_INVALID_REVNUM;
